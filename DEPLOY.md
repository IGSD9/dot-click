...
**Resend**（無料 100通/日）を Supabase に繋ぐと、Magic Link の上限が緩和されます。

> SMTP 設定は **Supabase Dashboard** で行います（コード変更不要）。

### 5-1. Resend アカウント作成

1. [resend.com](https://resend.com) → Sign up（GitHub 連携可）
2. Dashboard → **API Keys** → **Create API Key** → キーをコピー（`re_...`）

### 5-2. Supabase に SMTP を設定
...
| Sender email | Resend で確認済みのアドレス（初回は Dashboard の指示に従う） |
| Sender name | `Dot Click`（任意） |

**Save** 後、Dot Click から Magic Link を再送信して届くか確認。

### 5-3. 本番向け（任意）

Resend → **Domains** で独自ドメインを追加すると、スパム扱いされにくくなります。  
ドメインがなくても Resend 提供のアドレスでテスト・小規模配布は可能です。
