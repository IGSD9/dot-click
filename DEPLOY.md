...
| Name | Value | 例 |
|------|-------|-----|
| `DATABASE_URL` | Supabase **Database URI**（`postgresql://...`） | 下記参照 |
| `NEXT_PUBLIC_SUPABASE_URL` | Supabase **Project URL** | `https://ghisemscbexvededtzuy.supabase.co` |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | anon public key | `eyJ...` |

> ⚠️ **`DATABASE_URL` と `NEXT_PUBLIC_SUPABASE_URL` は別物です。**  
> Project URL（`https://...supabase.co`）を `DATABASE_URL` に入れるとランキングが表示されません。

`DATABASE_URL` の取得方法:

1. Supabase Dashboard → **IGSD9's Project** → **Settings** → **Database**
2. **Connection string** → **URI** を選択
3. `[YOUR-PASSWORD]` を DB パスワードに置き換え
4. Vercel 本番向け（推奨）: **Transaction pooler** タブの URI（ポート **6543**、`?pgbouncer=true` 付き）
...
### トラブルシュート: ランキングが表示されない

**症状:** ランキング画面に `postgresql://` で始まる必要がある、などのエラー

**原因:** Vercel の `DATABASE_URL` に Project URL（`https://...supabase.co`）が入っている

**対処:**

1. [Vercel](https://vercel.com) → dot-click プロジェクト → **Settings** → **Environment Variables**
2. `DATABASE_URL` を編集 → Value を `postgresql://...` の URI に差し替え（上記 Step 4 参照）
3. **Save** → **Deployments** → 最新デプロイの **⋯** → **Redeploy**

**確認:** https://dot-click.vercel.app/api/health/db が `{"ok":true,...}` になれば OK

### トラブルシュート: Can't reach database server at `:5432`

**症状:** ランキング画面に `Can't reach database server at db....supabase.co:5432`

**原因:** Vercel に **Direct connection**（ポート 5432）の URI を設定している。Vercel のサーバーレス環境からは Direct 接続できない。

**対処:**

1. Supabase Dashboard → **Settings** → **Database** → **Connection string**
2. **Transaction pooler** タブを選択（Direct connection ではない）
3. **URI** をコピー → パスワードを置き換え
4. Vercel の `DATABASE_URL` をこの URI に差し替え → **Redeploy**
...
Custom SMTP（Resend / Gmail）を設定すると Magic Link の上限が緩和されます。

> **コード変更不要。** Supabase Dashboard だけで設定します。

⚠️ **IGSD9's Project は Team Condition と Dot Click で共有しています。**  
Supabase の Custom SMTP を ON にすると **両方のアプリのメール送信** に影響します。  
Dot Click だけ変えたい場合は、Supabase プロジェクトを分けるか、標準メールのまま運用してください。

**Custom SMTP をやめる（標準に戻す）:** → [Step 5C: Custom SMTP を無効化](#step-5c-custom-smtp-を無効化resendgmail-をやめる)

**ドメインなしで手軽に:** → [Step 5B: Gmail SMTP](#step-5b-gmail-smtpドメイン不要)  
**本番・大量送信:** → [Step 5A: Resend SMTP](#step-5a-resend-smtp独自ドメインあり)

---

## Step 5C: Custom SMTP を無効化（Resend/Gmail をやめる）

他プロジェクト（Team Condition 等）へ影響が出た場合、Supabase 標準メールに戻します。

1. [Supabase Dashboard](https://supabase.com/dashboard) → **IGSD9's Project**
2. **Authentication** → **Emails** → **SMTP Settings**
3. **Enable Custom SMTP** を **OFF**
4. **Save**

これで Dot Click / Team Condition とも **Supabase 標準のメール送信** に戻ります。

| 項目 | 標準メールに戻した後 |
|------|----------------------|
| 送信上限 | 約 3〜4 通/時（プロジェクト全体） |
| 影響範囲 | 同じ Supabase を使う全アプリ |
| Resend アカウント | 削除しなくて OK（Supabase から切るだけ） |

Magic Link テストは **1時間に数回まで**。`email rate limit exceeded` が出たら時間を空けて再試行。
